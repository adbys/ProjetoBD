...
CREATE TABLE Rio( idRio INTEGER,
			      ...
			      idBacia INTEGER NOT NULL, --GERANDO RELACIONAMENTO BACIA 1:N RIO
				  PRIMARY KEY (idRio),
...
CREATE TABLE Acude( idAcude INTEGER,
				    ...
				    comprimento DECIMAL,
				    area DECIMAL,
				    idRio INTEGER NOT NULL, --GERANDO RELACIONAMENTO RIO 1:N AÇUDE
				    PRIMARY KEY (idAcude),
...
CREATE TABLE Posto_pluviometrico( idPostoPluviometrico INTEGER,
								  ...
								  estado VARCHAR(30),
								  idBacia INTEGER NOT NULL, --Gerando relacionamento bacia monitorada 1:N por posto pluviometrico
...
CREATE TABLE Contrib_p_pluviometrico_acude( idPostoPluviometrico INTEGER NOT NULL, -- GERANDO RELACIONAMENTO AÇUDE N:N POSTO PLUVIOMETRICO
...
CREATE TABLE Cota_area_volume( id INTEGER,
						       ...
						       area DECIMAL,
						       volume DECIMAL,
						       idAcude INTEGER NOT NULL, --Gerando a relacao acude tem cota_area_volume
						       PRIMARY KEY (id),
						       FOREIGN KEY (idAcude) REFERENCES Acude(idAcude)
			     			 );
...
CREATE TABLE Medicao_cota_diaria( idMedicaoDiaria INTEGER,
				  cotaAtual DECIMAL,
				  data DATE,
				  matricula INTEGER NOT NULL, --Gerando a relacao acude tem medicao diaria
				  idAcude INTEGER NOT NULL, --Gerando a relacao usuario realiza medicao diaria
...
CREATE TABLE Medicao_pluviometrica( idMedicao INTEGER,
								    idPostoPluviometrico INTEGER NOT NULL, --GERANDO RELACIONAMENTO POSTO PLUVIOMETRICO 1:N MEDICAO PLUVIOMETRICA
								    matricula INTEGER NOT NULL, --GERANDO RELACIONAMENTO USUARIO 1:N MEDICAO PLUVIOMETRICA
